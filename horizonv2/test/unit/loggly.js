...
describe('feature: loggly integration', function() {
	...
	it('should push target error response data to loggly', function(done) {
		...
		mock.contextGetVariableMethod.withArgs('apiproxy.name').returns('horizon');
		...
		mock.contextGetVariableMethod.withArgs('proxy.basepath').returns('/edgecontroller/v1');
		...
		moduleLoader.load(js, function(err) {
			...
			expect(logglyRequestArguments[0]).to.equal('https://demo-org.apigee.net/edgecontroller/v1/logs');
